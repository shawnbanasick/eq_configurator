...
const generateStep2html = () => {
  ...
  const qSortPattern = clone(appState.qSortPattern);

  // calc number of columns
  let uniquesArray = qSortPattern.filter(
    (item, i, ar) => ar.indexOf(item) === i
  );
  let numColumns = uniquesArray.length;

  // set default cell height
  let cellHeight = `viewHeight / parseInt(longestColumn.__text, 10)`;

  // use custom values for height
  if (useConstantHeight === true || useConstantHeight === "true") {
    cellHeight = `${heightValue}`;
  }

  // set default - sort grid width
  let viewWidth = `viewWidth`;

  // adjust sort grid width to custom values for cells
  if (useConstantWidth === true || useConstantWidth === "true") {
    viewWidth = `${widthValue * numColumns}`;
  }
...
export default generateStep2html;
